...
namespace FixMyCity.Controllers
{
    ...
    public class ComplaintChatController : Controller
    {
        ...
        /// <summary>
        /// Server-renders the full chat partial for initial modal/drawer load.
        /// Kept separate from GetThread (which returns JSON for polling) because
        /// the two callers have different needs: this one needs the full
        /// composer/dropzone markup wrapper, GetThread only needs new bubbles.
        /// textOnly lets the officer modal request the lighter, text-only
        /// composer (no attach button/dropzone) via the same partial.
        /// </summary>
        [HttpGet]
        public ActionResult ThreadPartial(int complaintId, bool textOnly = false)
        {
            try
            {
                var vm = _chatService.GetThread(complaintId, CurrentActorId, CurrentRoleId, 0);
                ViewBag.ChatTextOnly = textOnly;
                return PartialView("_ComplaintChat", vm);
            }
            catch (BusinessException ex)
            {
                return Content($"<div class='p-3 text-danger'>{System.Net.WebUtility.HtmlEncode(ex.Message)}</div>");
            }
        }

        /// <summary>
        /// Newer-than-sinceMessageId messages plus current IsChatOpen state, as JSON.
        /// Used by the partial's own polling loop (every 5s) — pass 0 for the
        /// full thread, or the last-seen ChatMessageId for incremental updates.
        /// </summary>
        [HttpGet]
...
namespace FixMyCity.Controllers
{
    ...
    public class ComplaintChatController : Controller
    {
        ...
        // Accepts up to ChatMaxAttachmentsPerMessage files per call (model
        // binder maps repeated <input type="file" name="files"> fields to
        // this array). Still one bubble per file — the response carries one
        // rendered partial per attachment, same envelope shape as GetThread.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult SendAttachment(int complaintId, System.Web.HttpPostedFileBase[] files)
        {
            try
            {
                var messages = _chatService.SendAttachments(complaintId, CurrentActorId, CurrentRoleId, files);
                var items = messages
                    .Select(m => new { chatMessageId = m.ChatMessageId, html = RenderPartialToString("_ChatMessageBubble", m) })
                    .ToList();
                return Json(new { success = true, messages = items });
            }
            catch (BusinessException ex)
            {
                return Json(new { success = false, message = ex.Message });
