...
namespace FixMyCity.service
{
    ...
    (ConfigurationManager.AppSettings["AllowedAttachmentExtensions"] ?? ".jpg,.jpeg,.png,.pdf,.doc,.docx")
        ...
        // Cap per submission (filing or editing), not cumulative across a
        // complaint's lifetime — matches the file picker's own limit, and
        // keeps a single request from being throttled by attachments added
        // in an earlier edit.
        private static readonly int MaxAttachmentsPerComplaint =
            Convert.ToInt32(ConfigurationManager.AppSettings["MaxAttachmentsPerComplaint"] ?? "5");
        ...
        public List<AttachmentViewModel> UploadAttachments(int complaintId, int consumerId, IEnumerable<HttpPostedFileBase> files)
        {
            ...
            // Reject the whole batch up front rather than uploading 5 and
            // failing on the 6th — avoids a partial save the user didn't ask for.
            if (validFiles.Count > MaxAttachmentsPerComplaint)
                throw new BusinessException(
                    $"You can attach up to {MaxAttachmentsPerComplaint} files at a time. You selected {validFiles.Count}.",
                    "TOO_MANY_FILES");
